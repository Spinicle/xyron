...
###
# NOTE
# QOL changes required in view_project()
# Make everything look neat and pretty
# Exception handling (eg: if there are no ch in proj, print text saying so, dont display table)
# Bug fixes
###
...
def create_project(): # REQUIRES RICH TEXT
    ...
    # Make project_table
    cursor.execute(f"create table {project_table}(ch_no int, tl varchar(10), pr varchar(10), ts varchar(10), qc varchar(10), status varchar(20))")
...
def view_project(): # REQUIRES RICH TEXT
    ...
    if choice == "add":
        ...
        # Commit to table
        sql_string = f"insert into {project_table} (ch_no) values (%s)"
        values = (ch_no,)
        cursor.execute(sql_string, values)
        xyron_sql.commit()
...
def delete_project():
    ...
    if confirm == True:
        ...
        cursor.execute(f"delete from {project_table} where project_id = {project_id}")
    else:
        pass
